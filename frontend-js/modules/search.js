...
export default class Search {
  ...
  sendRequest() {
    axios
      ...
      .catch(() => {
        alert('Hello the request failed from sendRequest');
      });
  }
        ...
        ${posts
          ...
          <span class="text-muted small">by ${
            post.author.username
              ...
              1}/${postDate.getDay()}/${postDate.getFullYear()}</span>
        </a>`;
          })
          .join('')}
    </div>`);
    } else {
      this.resultsArea.innerHTML = `<p class="alert alert-danter text-center shadow-sm">Sorry, we could not find any results for that search.</p>`;
    }
    this.hideLoaderIcon();
